...
{
    internal class F304LimitedConcurrencyTaskScheduler : TaskScheduler
    {
        [ThreadStatic]
        ...
        public F304LimitedConcurrencyTaskScheduler(int concurrencyLevel)
        {
        ...
        {
            lock (tasksList) return tasksList;
        }
        protected override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
        {
            if (currentThreadIsProcessingItems == false) return false;
            if (taskWasPreviouslyQueued == true) this.TryDequeue(task);
            return TryExecuteTask(task);
        }
        protected sealed override bool TryDequeue(Task task)
        {
            lock (tasksList) return tasksList.Remove(task);
        }
